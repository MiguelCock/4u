...
- **Flutter app structure**: `main.dart` composes three independent widgets — `LocationInfo` (location.dart), `SimpleCameraWidget` (camera.dart), `SimpleMapWidget` (map.dart) — all driven by the singleton `LocationService` (`lib/services/location_service.dart`), which exposes a broadcast `Stream<Position>` and a cached `lastPosition` that the widgets subscribe to independently rather than sharing state via a parent widget.
- **Data flow today**: the camera widget POSTs a multipart request (photo + lat/lng/accuracy fields) directly to a hardcoded backend URL in `camera.dart` — this predates the `BACKEND_URL` env var wiring, so when working on networking code check whether the hardcoded URL still needs to be replaced.
