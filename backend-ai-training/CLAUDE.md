...
- **Payload indexes are required for filtering, including filter-based delete** — this bit us once against the real project: `DELETE /index_anchor/{id}`'s `Filter(must=[FieldCondition(key="anchor_point_id", ...)])` 400s on a live Qdrant server ("Index required but not found for...") unless that field has an explicit payload index, even though it works fine against a mock. `_INDEXED_FIELDS = ["anchor_point_id", "building_id"]` (every field any `Filter()` in this file ever keys on) is passed to every `qdrant.ensure_collection(...)` call — `packages.qdrant.Qdrant.ensure_collection`'s `indexed_fields` param creates them unconditionally, not just when the collection is freshly created, since a collection that already existed before a field started being filtered on needs the same repair. `packages/tests/test_qdrant.py`'s live-connectivity test (the only test in this repo that talks to a real Qdrant server) reproduces this exact failure and proves the fix, since a mocked test structurally can't catch a real server's filtering constraints.
...
4. **`DELETE /index_anchor/{anchor_point_id}`** — filter-delete on `payload.anchor_point_id` (`Filter(must=[FieldCondition(key="anchor_point_id", match=MatchValue(value=...))])`), removing every one of that point's indexed photos in one call. Called when an anchor point is deleted, and also whenever one is saved with a non-`verified` status.
