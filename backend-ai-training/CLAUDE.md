...
Intended (from the root README, **not implemented**, described here only so it isn't confused with current behavior): (1) admin captures anchor point photos via the app's admin UI (`application/lib/admin/capture_screen.dart` + `capture_photo_screen.dart` — this part is real and working) → `backend-map-management` stores metadata in `anchor_points`/`anchor_point_photos` and the image in Storage; (2) this service would load those images, preprocess with OpenCV (resize/normalize), and run them through a pretrained EfficientNet-B0 (fixed feature extractor, no fine-tuning planned for the first pass) to get a 1280-dim embedding; (3) embeddings would be indexed in Qdrant via `packages.qdrant` alongside the anchor point's id/coordinates; (4) the feature extractor is meant to be trained once and not need retraining when new anchor points are added — only new embeddings get indexed. None of steps 2-4 exist yet.
