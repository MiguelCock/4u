...
8. **`AdminHomeScreen`** — `GET /anchor-points` (`backend-map-management`), listed by description/status; a floating action button opens `CaptureScreen`, and an app-bar menu opens `AddPlaceScreen`/`AddBuildingScreen`; all three refresh the list on return.
