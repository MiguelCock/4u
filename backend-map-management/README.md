...
```bash
cd ..
docker build -f backend-map-management/Dockerfile -t backend-map-management .
docker run -p 8000:80 --env-file backend-map-management/.env backend-map-management
```
...
|--------|-----------------------------|----------------------|----------------------------------------|
| GET    | `/`                         | —                    | Health check                           |
| POST   | `/anchor-points/upload-image` | multipart file field `file` | Upload an anchor point photo, get back its URL |
...
Example (upload the photo first, then create the anchor point with the returned URL):

```bash
curl -X POST http://localhost:8000/anchor-points/upload-image \
  -F "file=@/path/to/anchor.jpg;type=image/jpeg"
# => {"url": "https://<project>.supabase.co/storage/v1/object/public/anchor-points/anchor.jpg"}
...
curl -X POST http://localhost:8000/anchor-points \
  ...
  -d '{
    ...
    "location_type_id": 1,
    "floor": 0,
    "image_url": "https://<project>.supabase.co/storage/v1/object/public/anchor-points/anchor.jpg",
    "latitude": 6.2442,
    "longitude": -75.5812,
...
This is CRUD-stub level: it inserts/reads rows via `db.client.table(...)` directly (no admin-role auth check, no anchor point verification workflow). `POST /anchor-points/upload-image` uploads to a Storage bucket named `"anchor-points"`, which must already exist in your Supabase project — this service doesn't create it. See the root `CLAUDE.md` and this directory's `CLAUDE.md` for more context.
