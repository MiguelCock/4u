...
- `anchor_points.captured_by` → `profiles(id)`, owned by `backend-user-management`, and it's `NOT NULL` in `db_schema/anchor_points.sql`. **`AnchorPointCreate` (`app/models.py`) doesn't have a `captured_by` field at all** — a real insert against the actual schema would fail with a not-null violation. This is a genuine bug, not just a documentation gap; fix the model (require `captured_by: str`, the id of the admin who captured the point) before wiring this up to a real database.
...
1. **`POST /anchor-points`** — caller (intended: the app's not-yet-built admin screen) sends `AnchorPointCreate` (`building_id`, `location_type_id`, `floor`, `heading`, `image_url`, `latitude`, `longitude`, `altitude`, `location_description`) → inserted as-is into `anchor_points`. As noted above, this will fail against the real schema until `captured_by` is added to the model, and the image itself must already be hosted somewhere before this call — there's no upload step.
...
`AnchorPointCreate` is missing `captured_by` (see above) — the single most important gap to fix before this service can write real rows.
